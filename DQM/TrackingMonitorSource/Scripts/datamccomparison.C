...
void datamccomparison_align_sm(string year) {
...
void compareHisto(TCanvas* canvas, const vector<TFile*>& v, const vector<string>& lglist, const vector<string>& tokens, const string& yr) 
{
  ...
  if (yr == "2016pre") {
    ...
    datafolder = "DQMData/Run 302131/StandaloneTrackMonitor/Run summary/highPurityTracks/";
    yearinfo   = "2017"; 
    luminfo    = "0.12 fb^{-1} (13 TeV)";
  } 
  else if (yr == "2018") {
...
//  TString h_title = "Run 275376 : " + tokens[2];
  ...
  else legend11 = new TLegend(0.50,0.66,0.87,0.78);
  ...
  textonplot(0.28, 0.83, 0.40, 0.85, 42,  yearinfo);
  textonplot(0.66, 0.875, 0.83, 1.0, 42, luminfo);
  textonplot(0.10, 0.875, 0.18, 1.0, 62, "CMS");
  textonplot(0.24, 0.860, 0.34, 1.0, 52, " Preliminary");
...
void openFiles(vector<TFile*>& v1, vector<string>& v2, const char* filename, const string& yr ) {
  ...
  else if (yr == "2017") filename = "filelist_align_17_sm.txt";
